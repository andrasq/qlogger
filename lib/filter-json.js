 ...
 *
 * Copyright (C) 2014-2016.2019 Andras Radics
...
var timestamp = require('./format-timestamp');
...
function JsonFilter( options ) {
    ...
    this.encode = options && options.encode || JSON.stringify;
...
JsonFilter.prototype.filter = function filter( message, loglevel ) {
    ...
    // if (this.includeTime) bundle.time = (message && message.time !== undefined) ? 'placeholder' : timestamp.formatJsDateIsoString();        // TODO:
    if (this.includeTime) bundle.time = (message && message.time !== undefined) ? 'placeholder' : Date.now();
