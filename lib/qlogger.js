...
// QLogger.prototype.addRelay = function(relay) { this._relays.push(relay); return this; };     // TODO:
...
QLogger.prototype._logit = function _logit( msg, loglevel ) {
    ...
    // TODO: the _relays get the unfiltered string
    ...
    if (loglevel <= this._loglevel) {
        ...
        for (i=0; i<this._writers.length; i++) {
            this._writing += 1;
            this._writers[i].write(msg, function(err) {
                self._writing -= 1;
...
QLogger.createWriter = function createWriter( writerSpec, callback ) {
    ...
    try {
        ...
        case 'tcp':
            hostport = name.split(":");
            socket = net.connect({host: hostport[0], port: hostport[1]}, function() { callbackOrReturn(false, socket) });
